...
def infer_shift(dt: pd.Timestamp) -> str:
    ...
    targets = {"Mañana": 7 * 60, "Tarde": 13 * 60, "Noche": 19 * 60}
    return min(targets, key=lambda k: abs(minutes - targets[k]))
...
def process_attendance(df: pd.DataFrame):
    ...
    if missing:
        raise ValueError("No encontré las columnas requeridas. La planilla debe incluir NOMBRE, MARC. y ESTADO.")
        ...
        for _, row in group.iterrows():
            ...
            if estado == "M/Ent":
                ...
                if open_entry is None:
                    ...
                    obs = "La salida es anterior al ingreso" if delta.total_seconds() < 0 else ""
                    pairs.append({
                        "Nombre": nombre,
                        "Fecha ingreso": open_entry["Marc."].strftime("%d/%m/%Y"),
                        "Hora ingreso": open_entry["Marc."].strftime("%H:%M"),
...
def export_excel(raw_df: pd.DataFrame, pairs_df: pd.DataFrame, summary_df: pd.DataFrame) -> bytes:
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        for sheet_name, df in {"Marcaciones": raw_df, "Asistencia procesada": pairs_df, "Resumen": summary_df}.items():
